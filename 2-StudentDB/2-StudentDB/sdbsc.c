...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    if (get_student(fd, id, &temp_student) == NO_ERROR) {
    ...
    lseek(fd, (MAX_STD_ID - 1) * STUDENT_RECORD_SIZE, SEEK_SET);
...
int del_student(int fd, int id)
{
    // TODO
    student_t temp_student;
    if (get_student(fd, id, &temp_student) == SRCH_NOT_FOUND) {
...
int count_db_records(int fd)
{
    ...
    ssize_t read_size;
    while ((read_size = read(fd, &student, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE) {
        if (student.id != 0) {
            count++;
        }
    }
