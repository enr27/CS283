...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        if (io_size <= 0) {
            perror("Error: Connection lost or recv failed");
            free(io_buff);
            return ERR_RDSH_COMMUNICATION;
        }
        ...
        if (cmd_rc == ERR_EXEC_CMD) {
            send_message_string(cli_socket, "rdsh-error: command execution failed\n");
        } else if (cmd_rc == OK_EXIT || strcmp(io_buff, "stop-server") == 0) {
            send_message_string(cli_socket, "rdsh-exec: server stopping...\n");
            free(io_buff);
            return OK_EXIT;
        } else if (cmd_rc != 0) {
            char msg[128];
            snprintf(msg, sizeof(msg), "rdsh-exec: rc = %d\n", cmd_rc);
...
int send_message_string(int cli_socket, char *buff){
    ...
    bytes_sent = send(cli_socket, buff, len, 0);
    if (bytes_sent < 0) {
